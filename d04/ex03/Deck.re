module Card = {
  ...
  let allSpades = {
    let rec createAllSpades = (valueList: list(Value.t), result: list(t)) =>
      ...
      | [] => List.rev(result)
      | [h, ...t] =>
        createAllSpades(t, [newCard(h, Color.Spade), ...result])
      };
  ...
  let allHearts = {
    let rec createAllHearts = (valueList: list(Value.t), result: list(t)) =>
      ...
      | [] => List.rev(result)
      | [h, ...t] =>
        createAllHearts(t, [newCard(h, Color.Heart), ...result])
      };
...
let newDeck = () => {
  let shuffle = d => {
    ...
    let sortnd = List.sort(compare, nd);
    List.map(snd, sortnd);
  };
...
let toStringList = (deck: t) => {
  let rec stringList_aux = (deck_aux: t, result: list(string)) =>
    switch (deck_aux) {
    | [] => List.rev(result)
